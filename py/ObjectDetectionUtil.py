...
def get_im_dim(im):
    cwd = os.getcwd()
    os.chdir(os.path.dirname(im))
    im = cv2.imread(os.path.basename(im))
    os.chdir(cwd)
    w,h = im.shape[1], im.shape[0]
    return w,h

def write_im_dim(det_name, orig_im):
    os.makedirs(os.path.dirname(det_name), exist_ok=True)
    cwd = os.getcwd()
    os.chdir(os.path.dirname(det_name))
    im = cv2.imwrite(os.path.basename(det_name), orig_im)
    os.chdir(cwd)
